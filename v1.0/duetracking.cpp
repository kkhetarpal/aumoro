...
int main( int argc, const char** argv )
{
   ...
   sleep(1);
    ...
    for(;;)
    {
		...
		{
                	//fit a bounding rectangle .boundingbox //this is just a rectangle //say named box
                        
			
			  Rect boundbox = trackBox.boundingRect();

          		  int x1 = boundbox.x;
            		  int y1 = boundbox.y;
          		  int w1 = boundbox.width;
         		  int h1 = boundbox.height;


			//ground touching point 	
			int xx = x1 + w1/2;
			//y = box.y + box.height;  //grounding touching
			int yy= y1 + h1/2;   ///centre point
			//decision module //give commands 
                                //testing serial
                                /*command = "r";
   				cout << "Command: " << command << endl;
	                        send_via_port(&command, "string", 0);                     //sending string command
